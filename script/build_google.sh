...
make -s ARCH=arm64 O=out gki_defconfig -j$(nproc --all) SUBARCH=arm64 DTC_EXT=dtc
make -j$(nproc --all) O=out ARCH=arm64 SUBARCH=arm64 LLVM=1 LLVM_IAS=1 REAL_CC=clang CLANG_TRIPLE="aarch64-linux-gnu-" CROSS_COMPILE="aarch64-linux-gnu-" CROSS_COMPILE_ARM32="arm-linux-gnueabi-"
